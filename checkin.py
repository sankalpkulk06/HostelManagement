...
def check_in():
    ...
    def submit():
        ...
        print(name4)
        srn4 = str(srn_entry.get())
        print(srn4)
        dept4 = dept_combo.get()
        print(dept4)
        year4 = year_combo.get()
        print(year4)
        address4 = address_entry.get('1.0', 'end-1c')
        print(address4)
        std_phone4 = str(std_phone_entry.get())
        parent_name4 = parent_name_entry.get()
        parent_number4 = str(parent_number_entry.get())
        LG_name4 = LG_name_entry.get()
        LG_number4 = str(LG_number_entry.get())
        ...
        with open("studentinfo.csv", "a") as fh:
            ...
            writer.writerow(l)
        '''
        #-------------CHECKING IF SRN & NUMBER IS VALID---------------------
        if len(srn4) == 13 and len(std_phone4) == 10 and len(parent_number4) == 10 and len(LG_number4) == 10:
            with open("studentinfo.csv", "a") as fh:
                writer = csv.writer(fh,lineterminator = '')
                l = [name4, srn4, dept4, year4, address4, std_phone4, parent_name4, parent_number4, LG_name4, LG_number4, payment4, food4, block4]
                print(l)
                writer.writerow(l)
            done = tk.Label(root, text = "Submitted Successfully !!", font =("times new roman",12, "bold"))
            done.place(relx=0.35, rely=0.855, relwidth=0.3, relheight=0.03)
        else:
            error5 = tk.Label(root, text = "Error! Invalid SRN/Phone Number.", font =("times new roman",12, "bold"))
            error5.place(relx=0.35, rely=0.855, relwidth=0.3, relheight=0.03)
